// ==UserScript==
// @name            Nexus Download Fix NoBooHooSuck
// @description     Fixing the dumb new download system for nexusmods, with no occasional redirecting to buy premium page
// @namespace       randomtdev, root
...
// Fork changes: no occasional redirecting to buy premium page
    ...
    if (params.get('nmm')) {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          setTimeout(function () {
            SetButtonLabel(button, originalText)
            if (button) button.downloading = false
          }, 5000)
          AddButtonEvents()
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (data && data.url) {
            ...
            DisplayPopup(
              'DownloadFix script error',
              "Download Failed! Request went through, but couldn't get URL.\n\nAre you logged in?"
            )
          }
        },
...
function PatchButton(button) {
  ...
  button.addEventListener('click', function (e) {
    ...
    if (!button.downloading) {
      // Make sure we're not sending more requests while downloading; Won't work anyway.
...
function RemoveAdblockBanner() {
  if ($('.premium-banner').remove().length > 0) {
    // oh no not the premium banner
...
function InitializePatches() {
  if (PatchDownloadButtons()) {
    ...
    console.error(
      'Looks like we failed to do button patches without an exception some reason. No buttons?'
    )
  }
}
...
function AddButtonEvents() {
  var buttons = window
    .jQuery('.popup-btn-ajax > span:contains("download")')
    .parent()
    .magnificPopup({
      type: 'ajax',
      fixedContentPos: false,
...
function Initialize() {
  ...
  window.jQuery(document).ajaxComplete(function (e, request, settings) {
    ...
    if (settings.url.indexOf('ModFilesTab') != -1) {
      // Was the files tab just loaded?
  ...
  if (document.URL.indexOf('?tab=files') > -1) {
    // Do initial patches if we're loading the files page directly.
    InitializePatches()
    RemoveAdblockBanner()
    AddButtonEvents()
  }
